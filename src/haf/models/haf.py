...
import haf.models.backbones.gemma as _gemma
import haf.models.haf_config as _lap_config
from haf.models.model_adapter import CoTObservation
from haf.models.model_adapter import preprocess_observation
from haf.models.model_utils.metrics import compute_per_vqa_dataset_metrics
from haf.models.model_utils.metrics import compute_sample_specific_metrics
from haf.models.model_utils.metrics import compute_token_accuracy_metrics
from haf.models.model_utils.visualization import log_attention_mask_wandb as _log_attention_mask_wandb
...
class HAF(_pi0.Pi0):
    ...
    def __init__(self, config: _lap_config.HAFConfig, rngs: nnx.Rngs):
        ...
        # Continuous retro-embedding auxiliary (JEPA-style visual retrospection). INERT unless
        # retro_embedding_loss_weight > 0 — creating the head only when enabled keeps existing configs byte-identical.
        # See experiments/RECOVERABILITY.md §6. Head maps an LLM hidden state -> SigLIP embedding width so it can
        # regress the (frozen) SigLIP embedding of a retrospective anchor frame.
        if float(getattr(config, "retro_embedding_loss_weight", 0.0)) > 0.0:
            self.retro_head = nnx.Linear(paligemma_config.width, paligemma_config.width, rngs=rngs)

        # This attribute gets automatically set by model.train() and model.eval().
        self.deterministic = True

    def _configure_shared_training_attributes(self, config: _lap_config.HAFConfig) -> None:
        ...
        self.retro_embedding_loss_weight = float(getattr(config, "retro_embedding_loss_weight", 0.0))
        self.stop_retro_to_vlm_grad = bool(getattr(config, "stop_retro_to_vlm_grad", False))

    def retro_embedding_loss(self, llm_hidden, retro_image):
        """Continuous retro-embedding auxiliary: predict the (frozen) SigLIP embedding of a retrospective anchor
        frame from an LLM hidden state. `stop_retro_to_vlm_grad` toggles whether the aux SHAPES the VLM backbone
        (False = soft co-training, the point of the aux) or is insulated like the action expert (True = KI control).
        Returns a scalar cosine-distance loss. Only call when retro_embedding_loss_weight > 0.

        Args:
            llm_hidden: pooled VLM hidden state, shape [b, width].
            retro_image: retrospective anchor frame(s), same format as obs images, shape [b, H, W, 3].
        """
        img_tokens, _ = self.PaliGemma.img(retro_image, train=False)      # [b, n, width]
        target = jax.lax.stop_gradient(jnp.mean(img_tokens, axis=1))      # [b, width] frozen SigLIP target
        h = jax.lax.stop_gradient(llm_hidden) if self.stop_retro_to_vlm_grad else llm_hidden
        pred = self.retro_head(h)                                         # [b, width]
        pn = pred / (jnp.linalg.norm(pred, axis=-1, keepdims=True) + 1e-8)
        tn = target / (jnp.linalg.norm(target, axis=-1, keepdims=True) + 1e-8)
        return jnp.mean(1.0 - jnp.sum(pn * tn, axis=-1))                  # cosine distance
