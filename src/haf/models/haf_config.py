...
import haf.models.backbones.gemma3 as _gemma3
from haf.models.model_adapter import CoTObservation
from haf.models.model_adapter import ExtendedModelType

if TYPE_CHECKING:
    from haf.models.haf import HAF
    from haf.models.haf_gemma3 import HAFGemma3


@dataclasses.dataclass(frozen=True)
class HAFConfig(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    ...
    # --- Continuous retro-embedding auxiliary (JEPA-style visual retrospection) --------------------------
    # A regression head on the VLM hidden state predicts the (frozen) SigLIP embedding of a retrospective
    # target frame (an initial-scene anchor sampled from the first ~2s of the episode). This is the
    # low-recoverability *visual* auxiliary; see experiments/RECOVERABILITY.md §6. These flags are inert
    # (no-op) until the head is implemented in haf.py — kept here so config arms can reference them.
    #   weight > 0 activates it; stop_retro_to_vlm_grad toggles whether it SHAPES the backbone (False=soft,
    #   the point of the aux) or is insulated like the action expert (True=KI control).
    retro_embedding_loss_weight: float = 0.0
    stop_retro_to_vlm_grad: bool = False
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "HAF | HAFGemma3":
        """Create the appropriate model based on the variant."""
        if "gemma3" in self.paligemma_variant:
            from haf.models.haf_gemma3 import HAFGemma3

            return HAFGemma3(self, rngs=nnx.Rngs(rng))
        from haf.models.haf import HAF

        return HAF(self, rngs=nnx.Rngs(rng))
