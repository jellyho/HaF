...
from haf.policies.question_types import AnswerFormat
from haf.policies.question_types import QuestionConfig
from haf.policies.question_types import QuestionType
from haf.policies.question_types import compute_dominant_directions
from haf.policies.question_types import compute_displacement_magnitude
from haf.policies.question_types import compute_gripper_change
from haf.policies.question_types import compute_motion_magnitude
from haf.policies.question_types import compute_past_gripper
from haf.policies.question_types import compute_progress_phase
from haf.policies.question_types import format_delta_motion
from haf.policies.question_types import get_embodiment_name
from haf.policies.transforms.action_processor import ActionProcessor
from haf.policies.transforms.action_text import describe_language_action_scale
from haf.policies.transforms.action_text import is_idle_language_action
from haf.policies.transforms.text_utils import TextParser

if TYPE_CHECKING:
    from haf.policies.lang_action_formats import LanguageActionFormat
...
class PredictionSampleHandler:
    ...
    def _format_question_answer(
        ...
        # ---- Retrospective / input-masked question types (recoverability-lowering auxiliaries) ----
        # TASK_INFERENCE (input-side lever): the prompt is REPLACED by "what task?"; since prediction samples
        # already overwrite the instruction with the generated question (see .process), the instruction is
        # naturally masked from the input, and the answer IS the instruction. Works with existing plumbing.
        if question_type in (QuestionType.TASK_INFERENCE, QuestionType.TASK_INFERENCE_FROM_HISTORY):
            prompt = config.get_prompt_template(question_type, rng)
            answer = TextParser.parse_prompt(data)  # the (masked) instruction becomes the target
            return prompt, answer

        # PROGRESS_ESTIMATION: recover the temporal phase t/T. REQUIRES data["progress"] (see WIRING SPEC in
        # experiments/RECOVERABILITY.md §7 — plumb `progress` through add_prediction_pairs/output_schema).
        if question_type == QuestionType.PROGRESS_ESTIMATION:
            prompt = config.get_prompt_template(question_type, rng)
            return prompt, compute_progress_phase(float(data["progress"]))

        # DISPLACEMENT_FROM_START: distance moved since the (≈2s-window) start anchor. REQUIRES data["displacement_cm"].
        if question_type == QuestionType.DISPLACEMENT_FROM_START:
            prompt = config.get_prompt_template(question_type, rng)
            return prompt, compute_displacement_magnitude(float(data["displacement_cm"]))

        # PAST_GRIPPER_RECALL: gripper change since the start anchor. REQUIRES data["start_gripper"].
        if question_type == QuestionType.PAST_GRIPPER_RECALL:
            prompt = config.get_prompt_template(question_type, rng)
            return prompt, compute_past_gripper(float(data["start_gripper"]), gripper)

        # RETRO_MOTION: how the robot GOT here (past→current). REQUIRES the sample to be a RETROSPECTIVE pair
        # (past, current) with language_actions = the retro cumulative motion — so motion_components here already
        # describe the backward motion. See WIRING SPEC (retrospective pair in add_prediction_pairs).
        if question_type == QuestionType.RETRO_MOTION:
            answer_format = config.sample_answer_format(rng)
            prompt = config.get_prompt_template(QuestionType.RETRO_MOTION, rng, frame_description=frame_description)
