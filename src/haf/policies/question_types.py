...
class QuestionType(enum.Enum):
    ...
    # --- Retrospective (backward-looking) question types ---------------------------------------------
    # These recover the PAST of the current episode rather than predict the future. They are the
    # shortcut-free / hindsight objectives the probe study found regularize BC best (retro-obs was the
    # single best auxiliary). Unlike the foresight types above, their answers cannot be read off the
    # current frame alone — the model must encode what already happened.
    #
    # RETRO_MOTION: how the robot GOT to the current frame (cumulative motion from an earlier frame -> now)
    RETRO_MOTION = "retro_motion"
    # DISPLACEMENT_FROM_START: how far the end-effector has moved since the episode began
    DISPLACEMENT_FROM_START = "displacement_from_start"
    # PROGRESS_ESTIMATION: how far along the task is (temporal phase of the current frame)
    PROGRESS_ESTIMATION = "progress_estimation"
    # PAST_GRIPPER_RECALL: whether a gripper open/close already happened earlier in the trajectory
    PAST_GRIPPER_RECALL = "past_gripper_recall"

    # --- Input-masked task inference (JEPA-style INPUT-side recoverability lever) ---------------------
    # Instead of choosing a low-recoverability TARGET, we MASK an input (the language instruction) and ask
    # the model to recover the task. This turns the instruction from an input-shortcut into a target and
    # forces the representation to encode the task rather than copy it. It is low-recoverability ONLY where
    # the scene does not already reveal the task — measure per domain (cf. exp1 "R3 instruction"
    # recoverability: ~0 on DROID = a good aux, ~0.9 on LIBERO-object = trivial). Answer = the instruction.
    #
    # TASK_INFERENCE: infer the task from the current observation, instruction masked from the prompt.
    TASK_INFERENCE = "task_inference"
    # TASK_INFERENCE_FROM_HISTORY: infer the goal from the behavior/trajectory so far (more retrospective).
    TASK_INFERENCE_FROM_HISTORY = "task_inference_from_history"
...
# --- Retrospective prompt templates ------------------------------------------------------------------
# RETRO_MOTION: earlier frame -> current frame (the motion that BROUGHT the robot here).
RETRO_MOTION_PROMPTS = [
    "How did the robot get from the first image to the second{frame_ref}?",
    "Describe the motion the robot already made to reach the current frame{frame_ref}",
    "What movement brought the robot from the earlier observation to now{frame_ref}?",
    "Looking back, what action was taken between these two frames{frame_ref}?",
    "Reconstruct the robot's motion from the past frame to the present{frame_ref}",
    "What did the robot do to move from image 1 to image 2{frame_ref}?",
]
DISPLACEMENT_FROM_START_PROMPTS = [
    "How far has the robot's end-effector moved since the start of this episode?",
    "Compared to where it began, how much has the robot moved so far?",
    "Estimate the total displacement of the end-effector from its starting position",
    "Has the robot moved a small, moderate, or large distance since it started?",
    "How far is the gripper now from where the trajectory began?",
]
PROGRESS_ESTIMATION_PROMPTS = [
    "How far along is the robot in completing its task?",
    "Is the robot just starting, in the middle, or almost done with the task?",
    "Estimate the progress of the task shown in this frame",
    "What stage of the task is the robot currently at?",
    "How much of the task has the robot completed so far?",
]
PAST_GRIPPER_RECALL_PROMPTS = [
    "Earlier in this trajectory, did the gripper open or close?",
    "Has the gripper already changed state during this episode?",
    "Looking back over the trajectory so far, was there a gripper action?",
    "Did the robot open or close its gripper at some earlier point?",
    "Recall whether a grasp or release already happened in this episode",
]

# Input-masked task inference: the instruction is REMOVED from the prompt; the answer is the instruction.
TASK_INFERENCE_PROMPTS = [
    "What task is the robot performing?",
    "Describe the goal the robot is trying to accomplish.",
    "Based on this scene, what is the robot's task?",
    "What is the robot supposed to do here?",
    "Infer the instruction the robot is following.",
    "What is the intended task in this situation?",
]
TASK_INFERENCE_FROM_HISTORY_PROMPTS = [
    "Based on the robot's motion so far, what is it trying to do?",
    "Given the trajectory up to now, infer the robot's goal.",
    "From the behavior in these frames, what task is being performed?",
    "What goal explains the robot's actions so far?",
    "Looking at what the robot has done, what instruction is it following?",
    "Infer the task from the robot's recent motion.",
]

# Weight preset that mixes foresight + retrospective (hindsight) questions. Opt in via
# DataConfig.question_type_weights (defaults keep only the original foresight types). Requires the
# data-pipeline to build retrospective targets (past frame pairs / progress / displacement) — see
# base_dataset.add_prediction_pairs and the sample handler.
RETRO_MIXED_QUESTION_WEIGHTS = {
    QuestionType.DELTA_MOTION.value: 0.25,           # foresight
    QuestionType.RETRO_MOTION.value: 0.25,           # retrospective: how we got here
    QuestionType.DISPLACEMENT_FROM_START.value: 0.15,  # retrospective: distance since start
    QuestionType.PROGRESS_ESTIMATION.value: 0.15,    # retrospective: task phase
    QuestionType.PAST_GRIPPER_RECALL.value: 0.05,    # retrospective: past gripper event
    QuestionType.DIRECTION_CLASSIFICATION.value: 0.10,
    QuestionType.TASK_PREDICTION.value: 0.05,
}
...
# --- Retrospective answer generators -----------------------------------------------------------------


def compute_progress_phase(progress: float) -> str:
    """Qualitative task-progress phase from the normalized temporal position t/T in [0, 1]."""
    if progress < 0.2:
        return "just started"
    if progress < 0.45:
        return "early in the task"
    if progress < 0.7:
...
def compute_displacement_magnitude(total_cm: float) -> str:
    """Qualitative displacement-since-start bucket (larger thresholds than a single-step delta)."""
    if total_cm < 3.0:
        return "barely moved"
    if total_cm < 10.0:
        return "moved a small distance"
    if total_cm < 25.0:
        return "moved a moderate distance"
    return "moved a large distance"


def compute_past_gripper(gripper_start: float, gripper_now: float) -> str:
    """Whether the gripper already opened/closed earlier in the trajectory (start vs current)."""
    if gripper_now > 0.5 and gripper_start <= 0.5:
        return "the gripper opened earlier"
    if gripper_now <= 0.5 and gripper_start > 0.5:
        return "the gripper closed earlier"
    return "the gripper has not changed"
...
class QuestionConfig:
    ...
    def get_prompt_template(
        ...
        prompt_lists = {
            ...
            QuestionType.RETRO_MOTION: RETRO_MOTION_PROMPTS,
            QuestionType.DISPLACEMENT_FROM_START: DISPLACEMENT_FROM_START_PROMPTS,
            QuestionType.PROGRESS_ESTIMATION: PROGRESS_ESTIMATION_PROMPTS,
            QuestionType.PAST_GRIPPER_RECALL: PAST_GRIPPER_RECALL_PROMPTS,
            QuestionType.TASK_INFERENCE: TASK_INFERENCE_PROMPTS,
            QuestionType.TASK_INFERENCE_FROM_HISTORY: TASK_INFERENCE_FROM_HISTORY_PROMPTS,
        }
        ...
        # Format frame reference for two-frame motion prompts (foresight delta + retrospective motion)
        if question_type in (QuestionType.DELTA_MOTION, QuestionType.RETRO_MOTION) and "{frame_ref}" in template:
