...
import haf.models.haf_config as lap_config
import haf.models.model_adapter as _model_adapter
from haf.models.tokenizer import FASTTokenizer
from haf.models.tokenizer import Gemma3FASTTokenizer
from haf.models.tokenizer import Gemma3Tokenizer
from haf.models.tokenizer import PaligemmaTokenizer
import haf.policies.haf_policy as lap_policy
import haf.training.weight_loaders as weight_loaders
from haf.transforms import DetokenizeReasoning
from haf.transforms import ExtractFASTActions
from haf.transforms import TokenizeFASTInputs
from haf.transforms import TokenizePromptAndReasoning
...
class ModelTransformFactory(upstream_config.ModelTransformFactory):
    ...
    def _create_tokenizer(self, model_config: lap_config.HAFConfig, reasoning_mask_prob: float):
    ...
    def __call__(self, model_config: _model.BaseModelConfig) -> upstream_transforms.Group:
        if model_config.model_type == ModelType.LAP:
            assert isinstance(model_config, lap_config.HAFConfig)
            outputs = []
...
class RLDSDataConfig(BaseDataConfigFactory):
    ...
    def _create_data_transforms(
        ...
        if base_cfg.enable_diverse_questions:
            from haf.policies.question_types import QuestionConfig
...
_CONFIGS = [
    ...
    # ==================================================================================================
    # Exp 3 — HaF objective-mix ablation on RT-1, SCALED to the real VLA (pi05 recipe from a RAW PaliGemma
    # checkpoint, not a pi0/pi05 pretrained checkpoint). weight_loader defaults to kind="paligemma", so
    # every entry below initializes from gs://.../paligemma/pt_224.npz. Data is RT-1 alone
    # (fractal20220817_data). Evaluate closed-loop in SimplerEnv (google_robot) via scripts/simpler/main.py.
    #
    # The thesis: BC (action-only) memorizes and generalizes poorly; adding a shortcut-free auxiliary
    # (langact = language-action; prediction = world-model foresight) that shapes the VLM backbone regularizes
    # it. KI (stop_action_to_vlm_grad) is the STANDARD LAP/pi05 recipe (cf. configs `haf`, `pi05_replicated`),
    # so every aux-bearing arm keeps KI ON by default — the backbone is shaped by the aux (language/prediction)
    # and the action head is insulated. Only the pure-BC baseline turns KI off (there is no aux to shape the
    # backbone, so the action loss must). One soft (no-KI) mix arm is kept to test the probe finding that soft
    # co-training preserves vision-use better than hard KI. Ablation:
    #   exp3_rt1_bc        action only,          KI off  -- memorizer baseline (action shapes the whole VLM)
    #   exp3_rt1_langact   action + langact,     KI on   -- LAP standard (backbone = language, action insulated)
    #   exp3_rt1_pred      action + prediction,  KI on   -- world-model aux only shapes the backbone
    #   exp3_rt1_mix       action + langact+pred,KI on   -- full HaF mix, DEFAULT KI recipe
    #   exp3_rt1_mix_soft  action + langact+pred,KI off  -- soft co-training; hard-KI vs soft (probe finding)
    # batch_size/num_train_steps are set modestly; tune to your hardware (originals use 2048 on many hosts).
    # ==================================================================================================
    *[
        TrainConfig(
            name=_name,
            data=RLDSDataConfig(data_mix="fractal20220817_data", random_base_prob=0.5),
            model=lap_config.HAFConfig(
                action_dim=7,
                action_horizon=16,
                max_token_len=180,
                pi05=True,
                enable_action_training=True,
                enable_langact_training=_langact,
                enable_prediction_training=_pred,
                action_loss_weight=1.0,
                language_loss_weight=1.0,
                prediction_loss_weight=0.5,
                stop_action_to_vlm_grad=_ki,
            ),
            batch_size=256,
            num_train_steps=30_000,
        )
        for _name, _langact, _pred, _ki in [
            ("exp3_rt1_bc", False, False, False),
            ("exp3_rt1_langact", True, False, True),
            ("exp3_rt1_pred", False, True, True),
            ("exp3_rt1_mix", True, True, True),
            ("exp3_rt1_mix_soft", True, True, False),
        ]
    ],
    TrainConfig(
        name="haf_gemma3_4b",
